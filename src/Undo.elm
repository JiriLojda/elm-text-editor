module Undo exposing (RedoStack, UndoStack, applyChangeToUndoStack, redoLastBatch, undoLastBatch)
...
undoChange change model =
    case change of
        TextRemoved data ->
            let
                startingPos =
                    Pos.indexToCaretPos model.textValue data.startingIndex

                wasAfterCaret =
                    startingPos == data.originalCaretPosition

                newText =
                    insertAt data.startingIndex data.text model.textValue

                endingPos =
                    Pos.indexToCaretPos newText (data.startingIndex + String.length data.text)
            in
            { model
                | textValue = newText
                , caretPosition = data.originalCaretPosition
                , selection =
                    if data.wasSelected then
                        Just
                            { start =
                                if wasAfterCaret then
                                    endingPos
...
undoLastBatch model =
    let
        updatedModel =
            List.foldl undoChange model <| Maybe.withDefault [] (List.head model.undoStack)
    in
    { updatedModel
...
redoLastBatch model =
    let
        updatedModel =
            List.foldl redoChange model <| Maybe.withDefault [] (List.head model.redoStack)
    in
    { updatedModel
...
redoChange change model =
    case change of
        TextAdded data ->
            let
                newText =
                    insertAt data.index data.text model.textValue
            in
            { model
                | textValue = newText
...
applyChangeToUndoStack change model =
    let
        lastBatch =
            Maybe.withDefault [] <| List.head model.undoStack
    ...
    case change of
        Change.CaretMoved data ->
            ...
            { model
                | undoStack =
                    if data.toPosition == model.caretPosition then
                        model.undoStack
        ...
        Change.TextInserted data ->
            let
                ...
                adding =
                    TextAdded { index = index, text = data.toInsert }

                hasMoreThanOneChar =
                    String.length data.toInsert > 1
                ...
                shouldCreateNewBatch =
                    case lastChange of
                        Nothing ->
                            hasMoreThanOneChar || isNewLineChar

                        Just (TextAdded { text }) ->
                            hasMoreThanOneChar || isNewLineChar || text == "\n"
            ...
            { model
                | undoStack =
                    if shouldCreateNewBatch then
                        (adding :: selectionRemoval) :: model.undoStack

                    else
                        appendBatchToLatestBatch (adding :: selectionRemoval) model.undoStack
                , redoStack = []
            }
        ...
        Change.CharRemoved data ->
            ...
            { model
                | undoStack =
                    if shouldCreateNewBatch then
                        [ removalChange ] :: model.undoStack

                    else
                        appendToLatestBatch removalChange model.undoStack
                , redoStack = []
            }
    ...
    case model.selection of
        ...
        Just sel ->
            let
                ...
                endIndex =
                    Pos.caretPosToIndex model.textValue <| Sel.secondPosition sel
            in
            Just <|
                TextRemoved
