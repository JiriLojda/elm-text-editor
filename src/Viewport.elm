module Viewport exposing (Viewport, moveTo, moveBy, width, height, moveViewportIfNecessary, top, left, init, updateSize, updateContentSize, shouldViewVerticalScrollbar, shouldViewHorizontalScrollbar, contentWidth, contentHeight, changeWidthBy)
...
viewportAllowedOverscroll = 10


-- TYPES

type Viewport = Viewport ViewportInfo
...
heightInner viewport =
  if shouldViewHorizontalScrollbarInner viewport
      then viewport.height - viewport.scrollbarSize
...
widthInner viewport =
  if shouldViewVerticalScrollbarInner viewport
      then viewport.width - viewport.scrollbarSize
      else viewport.width
  ...
  if not (shouldMoveLeft viewport caretPos || shouldMoveRight viewport caretPos caretWidth)
    then
      ...
      if shouldMoveLeft viewport caretPos
        ...
        else caretPos.x - (widthInner viewport) + viewportCaretPadding + viewportOverMoveToCaretMargin
  ...
  if not (shouldMoveUp viewport caretPos || shouldMoveDown viewport caretPos)
    then
      ...
      if shouldMoveUp viewport caretPos
        ...
        else caretPos.y - (heightInner viewport) + viewportCaretPadding + viewportOverMoveToCaretMargin
